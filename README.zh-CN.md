...
一个具备完整工程闭环的健康测评体验：客户完成七步问卷，进度可中断恢复，获得个性化状态参考，并可解锁完整趋势报告。客户界面采用优雅运动感、高级简约的视觉语言，不展示任何开发或实现细节。
...
- **优雅运动感**：深森林绿、暖石色与能量青柠构成克制而有活力的视觉体系。
- **高级简约**：大字号编辑式排版、宽松留白和清晰层级，让客户始终聚焦自己的状态。
- **丝滑动效**：页面进入、步骤切换、轨道图形和趋势曲线采用统一缓动，并支持 `prefers-reduced-motion`。
- **客户语言**：落地页、问卷、加载、错误、锁定与完整报告只使用客户可理解的文案，不展示 API、数据库、算法版本、测试或支付实现细节。
- **全链路一致**：首页、测评准备页、七步问卷、基础报告和完整报告共享同一视觉系统，并兼顾移动端与桌面端。
